package com.checker.proxy.util;
...
public class CustomFormatter extends Formatter {
    ...
    private static final String format = "%1$tl:%1$tM:%1$tS %1$Tp %2$s: %3$s%4$s%n";
    private final Date dat = new Date();
    
    private int traceDepth;
    
    public CustomFormatter(int traceDepth){
    ...
    public synchronized String format(LogRecord record) {
        ...
        if (record.getThrown() != null) {
            ...
            pw.println();
            printLogTrace(pw, record.getThrown());
            pw.close();
	...
	private void printLogTrace(PrintWriter pw, Throwable t){
		int min = Math.min(t.getStackTrace().length, traceDepth);
		pw.println(t);
		for (int i = 0; i < min; i++) {
			pw.println("\tat " + t.getStackTrace()[i]);
		}
        // Print cause, if any
